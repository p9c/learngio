...
func extractAttributes(tag string) []*model.Attribute {
	rawAttrArray := attr.FindAllString(tag, -1)
	elementAttrs := []*model.Attribute{}
...
func ParseDocument(document string) *model.NodeDOM {
	DOM_Tree := &model.NodeDOM{
		Element:  "root",
		Content:  "THDWB",
		Children: []*model.NodeDOM{},
		Style:    nil,
		Parent:   nil,
	}
