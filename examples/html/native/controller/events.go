...
func attachBrowserWindowEvents(browserWindow *model.AppWindow) {
	//browserWindow.GioWindow.SetKeyCallback(func(w *glfw.Window, key glfw.Key, scanCode int, action glfw.Action, mods glfw.ModifierKey) {
...
func handleInputChars(key rune, browserWindow *model.AppWindow) {
...
func removeInputChars(browserWindow *model.AppWindow) {
	selectedElement := getSelectedUIElement(browserWindow.UIElements)
...
func handleEnterKey(browserWindow *model.AppWindow) {
	...
	if addressBarInput != nil && addressBarInput.Focused == true {
		if addressBarInput.Text != browserWindow.Location {
			...
			parsedDocument := ParseDocument(htmlString)
