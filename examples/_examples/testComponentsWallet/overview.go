...
import (
	...
	ncommand "github.com/gop9/olt/framework/command"
...
func newOverviewDemo() (od *overviewDemo) {
	...
	od.Text0Editor.Flags = framework.EditSimple
	od.Text0Editor.Maxlen = 64
	od.FieldEditor.Flags = framework.EditField
	od.FieldEditor.Maxlen = 64
	od.BoxEditor.Flags = framework.EditBox | framework.EditNeverInsertMode
	od.Text1Editor.Flags = framework.EditField | framework.EditSigEnter
	od.Text1Editor.Maxlen = 64

	od.edEntry1.Flags = framework.EditSimple
	od.edEntry1.Buffer = []rune("Menu Item 1")
	od.edEntry2.Flags = framework.EditSimple
	od.edEntry2.Buffer = []rune("Menu Item 2")
	od.edEntry3.Flags = framework.EditSimple
	od.edEntry3.Buffer = []rune("Menu Item 3")

	od.FitWidthEditor.Flags = framework.EditSimple
...
func (od *overviewDemo) overviewDemo(w *framework.Window) {
	...
	if w.TreePush(framework.TreeTab, "Window", false) {
		w.Row(30).Dynamic(2)
	...
	if w.TreePush(framework.TreeTab, "Widgets", false) {
		w.Row(20).Dynamic(1)
		if w.TreePush(framework.TreeNode, "Text", false) {
			od.overviewTextWidgets(w)
			w.TreePop()
		}

		w.Row(20).Dynamic(1)
		if w.TreePush(framework.TreeNode, "Button", false) {
			...
			w.Button(label.C(color.RGBA{0x30, 0x30, 0xcf, 0xff}), false)
			w.TreePop()
		}

		w.Row(20).Dynamic(1)
		if w.TreePush(framework.TreeNode, "Basic", false) {
			od.overviewBasicWidgets(w)
			w.TreePop()
		}

		w.Row(20).Dynamic(1)
		if w.TreePush(framework.TreeNode, "Selectable", false) {
			od.overviewSelectableWidgets(w)
			w.TreePop()
		}

		w.Row(20).Dynamic(1)
		if w.TreePush(framework.TreeNode, "Combo", false) {
			od.overviewComboWidgets(w)
			w.TreePop()
		}

		w.Row(20).Dynamic(1)
		if w.TreePush(framework.TreeNode, "Input", false) {
			...
			if w.Button(label.T("Submit"), false) || (active&framework.EditCommitted != 0) {
	...
	if w.TreePush(framework.TreeTab, "Popup", false) {
		od.overviewPopup(w)
		w.TreePop()
	}

	if w.TreePush(framework.TreeTab, "Layout", false) {
		od.overviewLayout(w)
		w.TreePop()
	}

	if w.TreePush(framework.TreeTab, "Image & Custom", false) {
		if od.Img != nil {
...
func (od *overviewDemo) overviewMenubar(w *framework.Window) {
	w.MenubarBegin()
	w.Row(25).Static(45, 45, 48, 48, 48)
...
func (od *overviewDemo) overviewTextWidgets(w *framework.Window) {
	w.Row(20).Dynamic(1)
	w.Label("Label aligned left", "LC")
	w.Label("Label aligned centered", "CC")
	w.Label("Label aligned right", "RC")
	w.LabelColored("Blue text", "LC", color.RGBA{0x00, 0x30, 0xcf, 0xcf})
	w.LabelColored("Yellow text", "LC", color.RGBA{0xff, 0xcf, 0x30, 0xcf})
...
func (od *overviewDemo) overviewBasicWidgets(w *framework.Window) {
	w.Row(30).Static(100)
...
func (od *overviewDemo) overviewSelectableWidgets(w *framework.Window) {
	w.Row(20).Dynamic(1)
	if w.TreePush(framework.TreeNode, "List", false) {
		w.Row(18).Static(130)
	...
	w.Row(20).Dynamic(1)
	if w.TreePush(framework.TreeNode, "Grid", false) {
...
func (od *overviewDemo) overviewComboWidgets(w *framework.Window) {
	w.Row(25).Static(200)
...
func (od *overviewDemo) errorPopup(w *framework.Window) {
	w.Row(25).Dynamic(1)
...
func (od *overviewDemo) overviewPopup(w *framework.Window) {
	// Menu contextual
	w.Row(30).Dynamic(1)
	...
	if w.Button(label.T("popup"), false) {
		w.Master().PopupOpen("Error", framework.WindowMovable|framework.WindowTitle|framework.WindowDynamic|framework.WindowNoScrollbar, rect.Rect{20, 100, 230, 150}, true, od.errorPopup)
	}
...
func (od *overviewDemo) overviewLayout(w *framework.Window) {
	w.Row(20).Dynamic(1)
	if w.TreePush(framework.TreeNode, "Widget", false) {
		btn := func() {
	...
	w.Row(20).Dynamic(1)
	if w.TreePush(framework.TreeNode, "Group", false) {
		groupFlags := framework.WindowFlags(0)
		if od.GroupBorder {
			groupFlags |= framework.WindowBorder
		}
		if od.GroupNoScrollbar {
			groupFlags |= framework.WindowNoScrollbar
		}
		if od.GroupTitlebar {
			groupFlags |= framework.WindowTitle
		}
		groupFlags |= framework.WindowNoHScrollbar
	...
	w.Row(20).Dynamic(1)
	if w.TreePush(framework.TreeNode, "Complex", false) {
		...
		if sw := w.GroupBegin("Group right top", framework.WindowBorder); sw != nil {
			sw.Row(18).Static(100)
			for i := range od.GroupSelected {
				txt := "Unselected"
				if od.GroupSelected[i] {
		...
		if sw := w.GroupBegin("Group right center", framework.WindowBorder); sw != nil {
			sw.Row(18).Static(100)
			for i := range od.GroupSelected {
				txt := "Unselected"
				if od.GroupSelected[i] {
		...
		if sw := w.GroupBegin("Group right bottom", framework.WindowBorder); sw != nil {
			sw.Row(18).Static(100)
			for i := range od.GroupSelected {
				txt := "Unselected"
				if od.GroupSelected[i] {
	...
	if w.TreePush(framework.TreeNode, "Splitter", false) {
		...
		if w.TreePush(framework.TreeNode, "Vertical", false) {
			...
			// Left column
			if sw := w.GroupBegin("left", framework.WindowNoScrollbar|framework.WindowBorder); sw != nil {
				sw.Row(25).Dynamic(1)
				sw.Button(label.T("#FFAA"), false)
				sw.Button(label.T("#FFBB"), false)
				sw.Button(label.T("#FFCC"), false)
			...
			if od.Resizing1 {
				if !w.Input().Mouse.Down(pointer.ButtonLeft) {
					od.Resizing1 = false
				} else {
			...
			// Middle column
			if sw := w.GroupBegin("center", framework.WindowNoScrollbar|framework.WindowBorder); sw != nil {
				sw.Row(25).Dynamic(1)
				sw.Button(label.T("#FFAA"), false)
				sw.Button(label.T("#FFBB"), false)
				sw.Button(label.T("#FFCC"), false)
			...
			if sw := w.GroupBegin("right", framework.WindowNoScrollbar|framework.WindowBorder); sw != nil {
				sw.Row(25).Dynamic(1)
				sw.Button(label.T("#FFAA"), false)
				sw.Button(label.T("#FFBB"), false)
				sw.Button(label.T("#FFCC"), false)
		...
		w.Row(20).Dynamic(1)
		if w.TreePush(framework.TreeNode, "Horizontal", false) {
			w.Row(30).Static(100, 100)
			w.Label("top:", "LC")
			...
			if sw := w.GroupBegin("top", framework.WindowNoScrollbar|framework.WindowBorder); sw != nil {
				sw.Row(25).Dynamic(3)
				sw.Button(label.T("#FFAA"), false)
				sw.Button(label.T("#FFBB"), false)
				sw.Button(label.T("#FFCC"), false)
			...
			if sw := w.GroupBegin("middle", framework.WindowNoScrollbar|framework.WindowBorder); sw != nil {
				sw.Row(25).Dynamic(3)
				sw.Button(label.T("#FFAA"), false)
				sw.Button(label.T("#FFBB"), false)
				sw.Button(label.T("#FFCC"), false)
...
func (od *overviewDemo) aboutPopup(w *framework.Window) {
	w.Row(20).Dynamic(1)
	w.Label("Nucular", "LC")
...
func (od *overviewDemo) showAppAbout(mw framework.MasterWindow) {
	var wf framework.WindowFlags

	if od.Border {
		wf |= framework.WindowBorder
	}
	if od.Resize {
		wf |= framework.WindowScalable
	}
	if od.Movable {
		wf |= framework.WindowMovable
	}
	if od.NoScrollbar {
		wf |= framework.WindowNoScrollbar
	}
	if od.Close {
		wf |= framework.WindowClosable
	}
	if od.Titlebar {
		wf |= framework.WindowTitle
	}
